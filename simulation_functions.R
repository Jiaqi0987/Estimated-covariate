...
simu_rslt = function(seed, config){
 ...
 # est_r = config$est_r
  ...
  or.lpoly <- if (fix.df.or.h == TRUE) {
    lprobust(y = y, x = r, p = poly_deg, eval = eval.pts,
             kernel = "gau", h = h_for("OR"))
  } else {
    lprobust(y = y, x = r, p = poly_deg, eval = eval.pts,
             kernel = "gau")
  }
  ...
  pi.lpoly <- if (fix.df.or.h == TRUE) {
    lprobust(y = y, x = rhat, p = poly_deg, eval = eval.pts,
             kernel = "gau", h = h_for("PI"))
  } else {
    lprobust(y = y, x = rhat, p = poly_deg, eval = eval.pts,
             kernel = "gau")
  }
  ...
  cpi.or.lpoly <- if (fix.df.or.h == TRUE) {
    lprobust(y = y-f.der_p*(a-rhat), x = rhat, p = poly_deg, eval = eval.pts, 
             kernel = "gau", h = h_for("CPI_or"))
  } else {
    lprobust(y = y-f.der_p*(a-rhat), x = rhat, p = poly_deg, eval = eval.pts, 
             kernel = "gau")
  }
  ...
  cpi.lpoly <- if (fix.df.or.h == TRUE) {
    lprobust(y = y-der.est*(a-rhat), x = rhat, p = poly_deg, eval = eval.pts, 
             kernel = "gau", h = h_for("CPI"))
  } else {
    lprobust(y = y-der.est*(a-rhat), x = rhat, p = poly_deg, eval = eval.pts, 
             kernel = "gau")
  }
  ...
  pi.cali.lpoly <- try(
    if (fix.df.or.h == TRUE) {
      lprobust(y = y_cali, x = rhat_cali, p = poly_deg, eval = eval.pts,
               kernel = "gau", h = h_for("PI_cali"))
    } else {
      lprobust(y = y_cali, x = rhat_cali, p = poly_deg, eval = eval.pts,
               kernel = "gau")
    },
  ...
  if (inherits(pi.cali.lpoly, "try-error")) {
    ...
    h_retry <- if (fix.df.or.h == TRUE) {
      h_for("CPI")
    } else {
      mean(cpi.lpoly$Estimate[, "h"], na.rm = TRUE)
    }
  ...
  bc.lpoly.or.cali <- bc.estimator(y = y, rhat = rhat_cali, a = a, muhat = mu,
                         eval = eval.pts, hseq = h_for("BC_or_cali"), deg = poly_deg, g_type = g_type)
  ...
  bc.lpoly.cali <- bc.estimator(y = y, rhat = rhat_cali, a = a, muhat = muhat,
                                   eval = eval.pts, hseq = h_for("BC_cali"), deg = poly_deg, g_type = g_type)
  ...
  bc.lpoly <- bc.estimator(y = y, rhat = rhat, a = a, muhat = muhat,
                                eval = eval.pts, hseq = h_for("BC"), deg = poly_deg, g_type = g_type)
  ...
  bc.lpoly.or <- bc.estimator(y = y, rhat = rhat, a = a, muhat = mu,
                              eval = eval.pts, hseq = h_for("BC_or"), deg = poly_deg, g_type = g_type)
